...
export function Result() {
  ...
  const { scans, bookmarkedProductIds, toggleBookmark } = useAppContext();
...
const scoreColor = score <= 40 ? 'text-brand-hazardous' : score <= 70 ? 'text-brand-caution' : 'text-brand-safe';
      ...
      <div className="p-5 flex flex-col gap-5 relative z-10">
        ...
        <div className={`rounded-3xl p-6 border text-center relative overflow-hidden shadow-2xl glass-card ${scoreBorderClass} ${scoreBgClass}`}>
          ...
          <div className="relative w-44 h-44 mx-auto mb-5">
            <svg viewBox="0 0 128 128" className="w-full h-full -rotate-90" style={{ overflow: 'visible' }}>
              {/* Background track */}
              <circle cx="64" cy="64" r={radius} fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="11" strokeLinecap="round" />
              {/* Animated score arc */}
              <circle
            ...
            <div className="space-y-3">
              {(() => {
                ...
                return allCodes.map(code => {
                  ...
                  const leftBorder = additive.hazard === 'hazardous' ? 'border-l-purple-500' : additive.hazard === 'caution' ? 'border-l-orange-400' : 'border-l-emerald-400';
                  return (
        ...
        <div className="glass-card rounded-3xl p-5 border border-white/5 shadow-xl">
          ...
          <div className="flex items-center gap-3 mb-4 overflow-x-auto no-scrollbar pb-1">
            {[
              { color: 'bg-purple-400', label: 'Avoid' },
              { color: 'bg-red-400', label: 'Harmful' },
          ...
          {product.ingredients.length === 0 || product.ingredients[0].includes('not clearly found') ? (
            ...
            <>
              <div className="flex flex-wrap gap-2">
                {(() => {
                  const riskOrder: Record<string, number> = { hazardous: 0, harmful: 1, moderate: 2, mild: 3, safe: 4 };
        ...
        {/* ── 8. Smart Verdict ── */}
        {scan.dietAdvice && (
          <div className={`rounded-3xl p-5 shadow-lg border mb-2 ${
