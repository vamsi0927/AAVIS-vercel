...
export function HealthDashboard() {
  ...
  // Calculate weekly grade based on avgScore
  let weeklyGrade = '-';
  if (scans.length > 0) {
  ...
  // Calculate hazardous counts and list avoided items dynamically
  const hazardousScans = scans.filter((s) => s.verdict === 'hazardous');
  ...
  const chartData = daysOfWeek.map(day => {
    const scansOnDay = scans.filter(s => {
      ...
      return isWithinRange && daysOfWeek[scanDate.getDay()] === day;
    });
    ...
    return { day, score: avgScoreOnDay };
  });
      ...
      <div className="px-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="glass-card border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
            ...
            <div className="text-[10px] text-content-secondary mt-0.5">Based on your scans</div>
          </div>
        </div>
            ...
            <>
              <div className="flex items-baseline gap-2 mb-1 mt-2">
                ...
                <span className="text-xs font-medium text-brand-safe/80 max-w-[150px] leading-tight">
                  products you chose not to buy this week
                </span>
              </div>
        ...
        {/* Keeping Hazardous card as requested (renamed) */}
        ...
        <div className="grid grid-cols-1 gap-4 mt-6">
          <button 
            ...
            className="bg-navy-800 border border-brand-primary/30 rounded-2xl p-4 flex items-center justify-center text-center gap-3 hover:bg-brand-primary/10 transition-colors"
          >
