// Fixed UMD wrapper for Vite/ESM compatibility
...
(function(root: any) {
	if(root.numeric) {
		...
		numeric.dim = function dim(x: any) {
    		var y,z;
    		if(typeof x === "object") {
        		y = x[0];
        		if(typeof y === "object") {
		...
		numeric._foreach2 = (function _foreach2(x: any,s: any,k: any,f: any): any {
		...
		numeric.diag = function diag(d: any) {
		...
		numeric.rep = function rep(s: any,v: any,k?: any): any {
		...
		numeric.identity = function(n: any) { return numeric.diag(numeric.rep([n],1)); };

		numeric.inv = function inv(a: any) {
		...
		numeric.dotMMsmall = function dotMMsmall(x: any,y: any) {
		...
		numeric.dotMV = function dotMV(x: any,y: any) {
    		var p = x.length, i;
		...
		numeric.dotVV = function dotVV(x: any,y: any) {
		...
		numeric.transpose = function transpose(x: any) {
...
function getNormalizationCoefficients(srcPts: number[], dstPts: number[], isInverse: boolean){
    ...
    try{
        matC = globalRoot.numeric.inv(globalRoot.numeric.dotMMsmall(globalRoot.numeric.transpose(matA), matA));
    }catch(e){
        console.log(e);
        return [1,0,0,0,1,0,0,0];
    }

    var matD = globalRoot.numeric.dotMMsmall(matC, globalRoot.numeric.transpose(matA));
    var matX = globalRoot.numeric.dotMV(matD, matB);
