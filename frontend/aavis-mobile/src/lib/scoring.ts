...
export function normalizeProduct(product: Product, warnings: string[]): { normalized: Product, confidence: number } {
  ...
  // Physical Limit Validation Layer
  // If scaling by factor results in physically impossible values, reset factor to 1
  const checkCalories = n.calories !== null ? n.calories * factor : 0;
  const checkSugar = n.sugar !== null ? n.sugar * factor : 0;
  const checkFat = n.fat !== null ? n.fat * factor : 0;
  const checkProtein = n.protein !== null ? n.protein * factor : 0;
  const checkCarbs = n.carbs !== null ? n.carbs * factor : 0;
  ...
  if (
    ...
    console.warn('[Scoring] Scaled values exceed physical limits. Falling back to factor = 1.');
    factor = 1;
    warnings.push("Serving size scaling resulted in physically impossible values. Showing raw values instead.");
  }
