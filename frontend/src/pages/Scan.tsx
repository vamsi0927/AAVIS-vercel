...
import { analyzeMultiStepScan, performOCR, getAiErrorMessage } from '../lib/aiAnalysis';
...
import { isAavisAIConfigured } from '../lib/apiConfig';
...
export function Scan() {
  ...
  const handleBarcodeScanned = async (barcode: string) => {
    ...
    setTimeout(() => {
      setScanError('Barcode database lookup is disabled. Please scan the ingredients and nutrition labels directly for a full AI analysis.');
      setIsScanning(false);
    }, 800);
  };
  ...
  const handleOcrStep = async (file: File, step: ScanStep) => {
    ...
    try {
      ...
      // Skip correction if AI was used and outputs perfect text.
      const hasAiKey = isAavisAIConfigured();
  ...
  const runFinalAnalysis = async (ingText: string, nutText: string | null) => {
    ...
    try {
      ...
      console.error('--------------------------');
      setScanError(getAiErrorMessage(error.message || 'UNKNOWN'));
      setIsScanning(false);
