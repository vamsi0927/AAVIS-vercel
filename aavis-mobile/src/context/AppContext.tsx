...
import {
  ...
  UserProfile,
  ScanResult,
  SavedMyth } from '../lib/types';
import { loadLocalPrefs, saveLocalPrefs, initLocalPrefs } from '../lib/storage';
...
export function AppProvider({ children }: {children: React.ReactNode;}) {
  ...
  // Initialize async settings on mount
  useEffect(() => {
    const initializeStorage = async () => {
      await initLocalPrefs();
      await initApiConfig();
      // Reload initial state with loaded preferences
      setState(prev => ({
        ...prev,
        ...getInitialState()
      }));
      ...
      if (cloudScans.length > 0) {
        setState(prev => {
          ...
          const cloudConverted: ScanResult[] = cloudScans.map(cs => {
            ...
            const product = {
              id: cs.id,
              name: ar.productName || cs.product_name || 'Scanned Product',
              brand: ar.brand || cs.brand || '',
              imageEmoji: '🤖',
              imageUrl: cs.image_url || ar.imageUrl || undefined,
              ingredients: ar.ingredients || cs.ingredients || [],
              productType: ar.productType || undefined,
              servingSize: ar.servingSize || undefined,
              rawNutrients: ar.rawNutrients || ar.nutrients || cs.nutrients || {},
              nutrients: ar.nutrients || cs.nutrients || {},
              additives: ar.additives || cs.additives || [],
              dynamicAdditives: ar.additiveDetails || {},
              dynamicIngredients: ar.ingredientDetails || {},
              allergens: ar.allergens || cs.allergens_detected || [],
            };
            
            const finalScore = cs.health_score !== null && cs.health_score !== undefined 
              ? cs.health_score 
              : (ar.finalScore !== undefined ? ar.finalScore : 50);

            const dynamicScore = computeHealthScore(product as any, currentProfile as any);
            const savedDimensions = ar.dimensions || ar.aiDimensions;
            
            let rawAdvice = ar.dietAdvice || cs.diet_advice || dynamicScore.dietAdvice || '';
            if (rawAdvice) {
              rawAdvice = rawAdvice.replace(/\(\d+\/100\)/g, `(${finalScore}/100)`);
            }
            ...
            return {
              ...
              date: cs.created_at,
              score: finalScore,
              verdict: cs.health_score !== undefined 
                ? (finalScore < 40 ? 'hazardous' : finalScore < 70 ? 'caution' : 'safe') 
                : (ar.verdict || dynamicScore.verdict),
              warnings: dynamicScore.warnings,
              product,
              aiSummary: ar.aiSummary || cs.ai_summary || undefined,
              dietAdvice: rawAdvice || undefined,
              scoreReasons: dynamicScore.scoreReasons,
              mainConcerns: ar.mainConcerns || dynamicScore.mainConcerns || [],
              personalizedWarnings: dynamicScore.personalizedWarnings,
              scoreBreakdown: ar.scoreBreakdown || dynamicScore.scoreBreakdown,
              aiDimensions: savedDimensions,
              overallAssessment: ar.overallAssessment,
              majorBenefits: ar.majorBenefits || []
            };
          });
  ...
  const saveMyth = async (mythData: Omit<SavedMyth, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
